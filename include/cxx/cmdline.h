...
cxx::CommandLine* cmd=cxx::CommandLine::instance();  if (!cmd->process(argc,argv,usage,minp,maxp)) return 1

#define PROCESS_COMMAND_LINE(usage)\
cxx::CommandLine* cmd=cxx::CommandLine::instance();  if (!cmd->process(argc,argv,usage,0,99999)) return 1
